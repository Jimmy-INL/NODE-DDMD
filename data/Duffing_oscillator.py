...
seed = 100
Y_train = Duffing_oscillator_ODE(x1range, x2range, 100000, tSpan, seed, "y")
filename_train = filenamePrefix + '_train_y.csv'
make_csv(filename_train, Y_train)
...
seed = 1
X_train = Duffing_oscillator_ODE(x1range, x2range, round(1 * numICs), np.arange(0, 0.25 + 0.1, 0.25), seed)
filename_train = filenamePrefix + "_E_eigfunc_confirm.csv"
make_csv(filename_train, X_train)
...
filenamePrefix = 'Linear'
seed = 10
X_train = Linear_ODE(x1range, x2range, 100000, np.arange(0, 2.5, 0.25), seed, "x")
filename_train = filenamePrefix + '_train_x.csv'
make_csv(filename_train, X_train)

seed = 10
tSpan = np.arange(0, 2.5 + 0.1, 0.25)
X_train = Linear_ODE(x1range, x2range, 100000, tSpan, seed, "y")
filename_train = filenamePrefix + '_train_y.csv'
make_csv(filename_train, X_train)
